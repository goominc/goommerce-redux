...
exports.removeBrandPendingOrder = removeBrandPendingOrder;
...
function order(state, action) {
  ...
  if (action.type === 'REMOVE_BRAND_PENDING_ORDER') {
    ...
    var _ret = (function () {
      ...
      var brandId = _action$payload.brandId;
      var orderProductId = _action$payload.orderProductId;

      var key = 'brands.' + brandId + '.pendingOrders';
      var list = _.filter(state[key], function (o) {
        return o.id !== orderProductId;
      });
      return {
        v: _.assign({}, state, (_$assign2 = {}, _$assign2[key] = list, _$assign2))
      };
    })();
...
function removeBrandPendingOrder(brandId, orderProductId) {
  return function (dispatch) {
    dispatch({
      type: 'REMOVE_BRAND_PENDING_ORDER',
      payload: { brandId: brandId, orderProductId: orderProductId }
    });
  };
}

function updateStock(orderProductId, count) {
  return _util.createFetchAction({
    type: 'UPDATE_STOCK',
